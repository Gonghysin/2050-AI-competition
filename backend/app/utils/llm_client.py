...
关于状态转换的重要规则：
1. 默认情况下，保持当前状态不变。当前状态是：{status}
2. 仅当用户明确表达答题意图时才将"status"设置为"quiz"，例如用户消息中包含：
   - "挑战"、"答题"、"测试"、"考考"、"出题"、"问题"等明确的答题请求
   - "我想回答问题"、"给我出道题"等明确的答题表述
3. 不要因为简单问候（如"你好"、"早上好"）或闲聊而改变状态
4. 如果已经处于"quiz"状态，除非明确结束答题，否则应保持在"quiz"状态
